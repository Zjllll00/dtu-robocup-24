...
class PlanTunnel(Node):
    ...
    def loop(self) -> None:
        ...
        if state == 130:
            if aruco.id == 29
                print("ArUco for tunnel detected!")
                state = 131
            elif
                print("No ArUco for tunnel...")
                break
        elif state == 131:
            Xc,Yc,Zc = ...
            if Zc <= 0.2 #in meters
            ...
            sleep(4)
            self.move_cmd.velocity = 0.0
            print("Open the tunnel gate...")
            # left backward with 70 degree and radius 55 cm (calculate the cirle)
            self.move_cmd.turn_rate = 0.1
            self.move_cmd.velocity = -0.429
            sleep(math.pi/0.39)
            self.move_cmd.turn_rate = 0.0
            print("Pass the tunnel")
            # foward 60 cm
